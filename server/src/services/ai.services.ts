...
export const detectContactType = async ( contractText:string) :Promise<string> => {
    const prompt =`Analyze the following contract and determine the type of contract it is. Provide only the contract type as a single string (e.g , "Employment", "Non-Disclosure","Sales","Lease",ect. ).  Do not include any addintional explanation or text.)
